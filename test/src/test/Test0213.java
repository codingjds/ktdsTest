...
//	    List에 이미 추가된 List 인 경우 다른 랜덤 숫자를 생성해 추가한다.
		...
		for(int i = 0; i < sixNum.size(); i++) {
				...
				sixNum.add(random.nextInt(45)+1);			
		}
...
//	     예를 들어 1시간 20분 주차를 했다면 80 으로 입력한다.
		...
		List<String> parkingMgnt = new ArrayList<>();
		Random random1 = new Random();
		
		System.out.println("주차할 차의 종류를 입력하세요: ");
		Scanner scan = new Scanner(System.in);
		String car = scan.nextLine();
		parkingMgnt.add(car);
		if(parkingMgnt.size() < 35) {
			parkingMgnt.add(car);
			System.out.println(car + " 주차되었습니다.");
		}
		else {
			System.out.println("만차되어 주차할 수 없습니다.");
		}
		int time2 = random1.nextInt(100)+1;
		System.out.println(time2);
		String customer = test0213.PriceByCar(car, time2);
...
//	    예> 95를 입력하면 "1시간 35분" 으로 출력한다.	
		System.out.println("(분 -> 시간) 시간을 입력하세요: ");
		int time3 = scan.nextInt();
		String time4 = time3/60 + " 시 " + time3%60 + " 분 ";
...
//    정수형 파라미터 1개, 실수형 파라미터 1개를 받는 메소드를 만든다.
//    두 파라미터를 곱한 결과를 정수형으로 변환해 반환하는 메소드를 생성하고 실행 및 결과를 출력해본다.
	public int MixMultifly(int a, double b) {
		int c = a*(int)b;
		return c;
	}
//12. 메소드 문제 4
//    정수형 파라미터 3개를 받는 메소드를 만든다.
//    전달 받은 파라미터의 값을 모두 곱해 출력만 하는 메소드를 생성하고 실행 및 결과를 출력해본다.	
	public int TripleMultifly(int a, int b, int c) {
		int d = a*b*c;  
		return d;
	}
//ds: 편의상 별도의 클래스를 만들지않음	
//	13. 클래스 문제 1
//    https://www.goodgag.net
//    게시글의 정보를 표현하는 클래스를 만들어 본다.
//    * 모든 Instance Field는 private 으로 만들어야 하며 Getter / Setter 가 반드시 생성되어 있어야 함.
...
//     예를 들어 1시간 20분 주차를 했다면 80 으로 입력한다.	
	...
	public String PriceByCar(String car, int time2) {
		int price = 0;
		if (car.equals("경차")) {
			price = (500*time2)/10;
		}
		else if(car.equals("소형차")) {
			price = 800*time2/10;
		}
		else if(car.equals("준중형차")) {
			price = 850*time2/10;
		}
		else if(car.equals("중형차")) {
			price = 1000*time2/10;
		}else if(car.equals("대형차")) {
			price = 1500*time2/10;
		}
		return "차종: " + car +" 주차요금: "+ price + " 주차시간: "+ time2;
	}
